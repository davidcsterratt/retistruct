...
just need R for Mac, available from http://www.r-project.org/. To
install the GUI with Mac can be more complex at present, depending on
the version of the Mac OS. 
...
\begin{enumerate}
\item If the previous step works, from within R type:
\begin{verbatim}
   source("install-gui.R")
\end{verbatim}
\item If all works, an interface window should appear.
\end{enumerate}
