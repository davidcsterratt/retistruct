...
compute.potential <- function(p1, p2, q) {
  ...
  E <- 0.05/r0*(atan(s2/r0) - atan(s1/r0))
...
compute.gradient <- function(p1, p2, q, d=0.1) {
  ...
  dEds1 <- 1/r1 * (-d/2/r1 + 1)
  dEds2 <- 1/r2 * (d/2/r2 - 1)
  gr <- -dEdr0 * (q - p0)/r0 + (dEds1 + dEds2) * v
  ...
  for (time in 1:tmax) {
    ...
    for (i in 1:N) {
      ...
      if (nrow(Cus)>0) {
        ...
        q <- qs[i,,drop=FALSE]
        FDs <- compute.gradient(p1, p2, q, d)
##         print(i)
##         print(q)
##         print(p1)
##         print(p2)
...
##         print(FDs)
    ...
    ## To compensate for the reaction, ensure that the force on the entire structure is zero 
