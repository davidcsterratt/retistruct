...
\marginpar{Between 250 and 300 words; currently 261}
...
\todo{DCS to Write author summary}
...
\todo{DCS to Insert correct download location}. The method is estimated to be
...
\todo{DL/IDT to provide muscle insertion data and DCS to analyse}

\todo{DL to revised figure with images at top and grouped data at bottom.}

\todo{DL/IDT to write text}

\todo{DCS to implement Wulff projection.}
...
\todo{DL/IDT to drive text and figures, with help possibly required
  from DCS.}
...
\todo{DCS to try elevation 35, azimuth 60 deg OD from Drager 1978 and to infer the origin of the optic axis by computing location of OD from adult data.}

\todo{DCS to work out how to rotate the axes.}

\todo{DCS to tidy contours by removing coords around edge. DCS to produce visuotopic representation.}

\todo{DCS to think about optimisation.}

\todo{DL/IDT to find good retinae.}
...
\todo{DL to send existing s-opsin images to DCS. DL would like to do
  some more.}
...
\todo{Describe what we have learnt by using the method}
...
program. \todo{REF for mosaic analysis and for tesselation on sphere.}
...
% SMI-32R; Covance) \todo{see figure}.
...
% dimensions perpendicular to an expansion. A perfectly incopressible
...
\todo{DL/IDT to supply supplementary figure}. The front of the eye was
...
% \citet{DiggFish85sphe} note that this method is sensitive to
% outliers, which tend to decrease the value of $\kappa$
% artificially. To gaurd against this, they compute an alternative
% estimate of $\kappa=c n^{1/3}$ where $c$ is a robust estimator of
% $\kappa$ that is computed from all the data.
...
\cite{DragHube76topo} \todo{Check DCS's reading of Dr\"{a}ger and Hubel}.
...
% \paragraph{Mosaic construction}
% \label{retistruct_plos:sec:mosaic-construction}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Overview of the method.} \textbf{(A)} The raw data:
    ...
    \todo{DL to mark up better to avoid gap, or provide better
      image.}}
...
\begin{figure}[!ht]
  ...
  \includegraphics{retistruct-phi0}
  \includegraphics{retistruct-titration}
  \caption{\textbf{Measurement of rim latitude.} \textbf{(A)}~The
    ...
    around the optimal rim angle. \todo{DCS to investigate outliers.}}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Examples of reconstructed retinae.}
    ...
    for corresponding panel in \textbf{(A--D)}. \todo{DCS to write sentence or paragraph about cutoff for bad retinae.}}
  \label{retistruct_plos:fig:examples}
\end{figure}

\begin{figure}[!ht]
  \centering
  \includegraphics{retistruct-goodness}
  \caption{\textbf{Measures of error of reconstructions.}
    ...
    $7.29^\circ$. \todo{DCS to investigate outliers.}}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Reconstruction of retinal image and grid lines.}
    Retina stained with SMI32. \todo{Put more more details in caption.}}
...
% LocalWords:  colliculus geniculate vivo center RG RGC SMI ImageJ Hjorth DCS
% LocalWords:  Lyngholm DL IDT hoc Hubel Diggle DCS's ger ccc Wulff ipsi LGN
% LocalWords:  dLGN DCL
