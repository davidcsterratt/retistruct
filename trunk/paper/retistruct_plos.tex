...
\item The triangulation points are then projected roughly onto a
  ...
  same age as the retina under reconstruction. The radius of the
  ...
  curtailed sphere. 
\item The optimal projection onto sphere is inferred by changing
  location of vertices on sphere so as to minimise the sum of
  normalised squared differences between lengths of corresponding
  connections in flattened and spherical retina whilst maintaining the
  correct orientation of triangles
  (Figure~\ref{fold-sphere:fig:method}E).
  \begin{equation}
  ...
  $i\in\mathcal{C}$ in flattened and spherical retina and $A_j$ and
  $a_j$ are signed areas of corresponding triangles $j\in\mathcal{T}$
  in flattened and spherical retina.  The signed area is positive for
  triangles in correct orientation, but negative for flipped
  triangles.  Parameter $k$ should be large so that only triangles
  with negative or near-zero area ratios are penalised. Parameter
  $\alpha$ scaled according to number of connections.
\item The locations of datapoints and landmarks on the sphere are
  determined (Figure~\ref{fold-sphere:fig:method}F). To do this, for
  each datapoint the its barycentric coordinates within its containing
  ...
  the locations of lines of lattitude and longitude in the flat
  retina.
\end{enumerate}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Overview of the method.} \textbf{(A)} The raw data:
    ...
    pair of red and orange lines indcate the vertices and apex of the
    five tears. Note that tear 2 is a subtear of tear 1. \textbf{(C)}
    The outline after triangluation (shown by grey lines) and
    stitching, indicated bby blue lines between corresponding points
    on the tears. Green lines indicate connections beteween tear
    vertices.  \textbf{(D)} The initial projection of the triangulated
    and stitched outline onto a hemisphere. Tears are show in
    white. Below is a representation of lines of lattitude and
    longitude on the flat outline. \textbf{(E)} The projection of the
    flat retina onto the sphere and the lines of lattitude and
    longitde after optimisation of the mapping. \textbf{(F)} The data
    reprsented on a polar plot of the reconstrcted retina. Mean
    locations of the different types of datapoints are indicated by
    filled diamonds. The nasal (N), dorsal (D), temporal (T) and
    ventral (V) poles are indicated. For reference, data is plotted on
    the flat reresentation below. }
...
% LocalWords:  Sterratt Nedergaard MRC MacOS Retistruct
